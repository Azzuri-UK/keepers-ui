        ...
        <template v-slot:default="props">
            <v-hover :key="item.id" v-for="item in props.items" v-slot:default="{ hover }">
                ...
                <v-card color="orange"  :class="item.raid_status === 0 ? 'closedRaid' : 'openRaid'"
                        v-on:click="viewList(item.id,item.name,item.type)" :elevation="hover ? 12 : 2"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            viewList: function(listId,item,type){
                this.$router.push({name: 'list', params: {listId: listId, listItem: item,listType: type}})
            }
        }
    }
</script>
